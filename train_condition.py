...
def remove_overlap(seg_out, warped_cm):
    ...
    print("remove_overlap is running")
    return warped_cm
...
def train(opt, train_loader, test_loader, val_loader, board, tocg, D):
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        if opt.occlusion:
            ...
            # print("912345678",warped_clothmask_paired)
        ...
        torchshow.show(warped_cloth_paired)    
        
        # generated fake cloth mask & misalign mask
        
        fake_clothmask = (torch.argmax(fake_segmap.detach(), dim=1, keepdim=True) == 3).long()
        # print("FAKE",fake_clothmask)
        # torchshow.show(fake_segmap)
        misalign = fake_clothmask - warped_cm_onehot
        misalign[misalign < 0.0] = 0.0
        # plt.imshow(fake_clothmask[0].permute(1, 2, 0)  )
        # print("12345678",fake_clothmask)
        #This is your tensor
        # tensor = fake_clothmask*255
        # tensor = np.array(tensor, dtype=np.uint8)
        # if np.ndim(tensor)>3:
        #     assert tensor.shape[0] == 1
        #     tensor = tensor[0]
        # print(PIL.Image.fromarray(tensor))
        # arr_ = np.squeeze(fake_clothmask) # you can give axis attribute if you wanna squeeze in specific dimension
        # plt.imshow(arr_)
        # plt.show()
        ...
        # print("123456789",warped_cloth_paired)
